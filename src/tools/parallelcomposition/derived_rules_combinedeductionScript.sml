  ...
  ``∀t Sym P S1 S2 Sym' P' S1' S2' (MTrn1:('event1 + 'eventS, ('symb pred1), 'state1, 'symb) mtrel) (MTrn2:('event2 + 'eventS, ('symb pred2), 'state2, 'symb) mtrel) (ded1:('symb pred1) tded) (ded2:('symb pred2) tded).
     ...
     gen_tac >> Cases_on `h` >- (
      ...
      RES_TAC >> Q.EXISTS_TAC `NONE::t1` >> Q.EXISTS_TAC `NONE::t2` >>
        ...
        metis_tac[DedRelAll,binterl_none] 
      )   >>         
     Cases_on `x` >-(
      Cases_on `x'` >-(
...
val binterleave_trace_decomp_to_comp_combinededuction_thm = store_thm(
  ...
  ``∀t Sym P S1 S2 Sym' P' S1' S2' (MTrn1:('event1 + 'eventS, ('symb pred1), 'state1, 'symb) mtrel) (MTrn2:('event2 + 'eventS, ('symb pred2), 'state2, 'symb) mtrel) (Ded1:('symb pred1) tded) (Ded2:('symb pred2) tded). 
     ...
     FULL_SIMP_TAC (list_ss++pred_setSimps.PRED_SET_ss++boolSimps.LIFT_COND_ss++boolSimps.EQUIV_EXTRACT_ss) [symbolicParlComp_def] >> rpt strip_tac >> IMP_RES_TAC binterl_moveNONE >> rw[] >> IMP_RES_TAC TranRelSnocRevNone>>
     Q.EXISTS_TAC `P''` >>  reverse(rw[]) >> PAT_X_ASSUM ``!Sym P S1 S2 Sym' P' S1' S2' MTrn1 MTrn2 Ded1 Ded2. A`` (ASSUME_TAC o (Q.SPECL [`Sym`,`P`,`S1`,`S2`,`Sym'`,`P''`,`S1'`,`S2'`,`MTrn1`,`MTrn2`,`Ded1`,`Ded2`])) >> IMP_RES_TAC binterl_movenone >> RES_TAC >> cheat
  );

(*
val binterleave_trace_emptydeduction_thm = store_thm(
  "binterleave_trace_emptydeduction", ``
                       ∀t Sym P S1 S2 Sym' P' S1' S2' (MTrn1:('event1 + 'eventS, 'pred1, 'state1, 'symb) mtrel) (MTrn2:('event2 + 'eventS, 'pred2, 'state2, 'symb) mtrel) (Ded1:('pred1) tded) (Ded2:('pred2) tded). 
                         ...
                         ⇔
                           ...
                           ``,
                           rpt gen_tac >> EQ_TAC >>
                       rewrite_tac[binterleave_trace_comp_to_decomp_emptydeduction_thm] >>
                       rewrite_tac[binterleave_trace_decomp_to_comp_emptydeduction_thm]
  );

val binterleave_composition_emptydeduction_thm = store_thm(
  "binterleave_composition_emptydeduction", ``
...
``,
rewrite_tac[binterleave_ts,traces_def,comptraces_def,EXTENSION] >> rw[] >> rewrite_tac[binterleave_trace_emptydeduction_thm]   
  ); 
  

  
val compose_vs_modules_emptydeduction_thm = store_thm(
  "compose_vs_modules_emptydeduction_thm", ``
  ...
  ,
  rewrite_tac[binterleave_composition_emptydeduction_thm,binterleave_ts] >>
