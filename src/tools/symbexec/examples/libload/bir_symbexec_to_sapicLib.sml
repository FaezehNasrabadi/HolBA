structure bir_symbexec_to_sapicLib =
struct
...
fun BExp_to_IMLExp vals_list exec_sts pred_be =
	...
	val result = if (is_BExp_Const pred_be) then
			 (if (is_Imm128 o dest_BExp_Const) pred_be then
			      Con (Name (PubName ,("c_"^((Arbnum.toString o wordsSyntax.dest_word_literal o dest_Imm128 o dest_BExp_Const) pred_be))))
			  else if (is_Imm64 o dest_BExp_Const) pred_be then
			      Con (Name (PubName ,("c_"^((Arbnum.toString o wordsSyntax.dest_word_literal o dest_Imm64 o dest_BExp_Const) pred_be))))
			  else if (is_Imm32 o dest_BExp_Const) pred_be then
			      Con (Name (PubName ,("c_"^((Arbnum.toString o wordsSyntax.dest_word_literal o dest_Imm32 o dest_BExp_Const) pred_be))))
			  else if (is_Imm16 o dest_BExp_Const) pred_be then
			      Con (Name (PubName ,("c_"^((Arbnum.toString o wordsSyntax.dest_word_literal o dest_Imm16 o dest_BExp_Const) pred_be))))
			  else if (is_Imm8 o dest_BExp_Const) pred_be then
			      Con (Name (PubName ,("c_"^((Arbnum.toString o wordsSyntax.dest_word_literal o dest_Imm8 o dest_BExp_Const) pred_be))))
			  else if (is_Imm1 o dest_BExp_Const) pred_be then
			      Con (Name (PubName ,("c_"^((Arbnum.toString o wordsSyntax.dest_word_literal o dest_Imm1 o dest_BExp_Const) pred_be))))
			 ...
			 (if identical “BType_Bool” ((snd o dest_BVar o dest_BExp_Den) pred_be) then
			  ...
			  else (Var ((rev_name o fst o dest_BVar_string o dest_BExp_Den) pred_be)))
...
fun path_to_process event_names vals_list refine_preds exec_sts [] =
    (ProcessNull)
  | path_to_process event_names vals_list refine_preds exec_sts (pred::preds) =
    let
	(*val _ = print ((pred)^"\n");*)
...
fun sym_exe_to_SAPIC exec_sts =
	...
	(*val init_process = ProcessNull;*)
	    
	val Sapic_processes = path_to_process event_names vals_list refine_preds exec_sts (rev refine_preds);
    
    in
	SAPIC_to_file (Process_to_string Sapic_processes)
    end;
